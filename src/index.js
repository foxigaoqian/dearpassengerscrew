...
function deepContentPage(kind, slug) {
  ...
  const sections = item.sections.map((section,index)=>{
    ...
    return `<section><p class="kicker">${String(index+1).padStart(2,"0")} · ${kind === "news" ? "REPORT" : "BRIEFING"}</p><h2>${esc(section.heading)}</h2>${section.paragraphs.map((text)=>`<p class="long-copy">${esc(text)}</p>`).join("")}${bullets}</section>`;
  }).join("");
  ...
  <link rel="canonical" href="${canonical}"><link rel="alternate" hreflang="en" href="${canonical}"><link rel="alternate" hreflang="x-default" href="${canonical}">
