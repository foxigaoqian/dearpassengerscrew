...
function resourceCards(locale) {
  ...
  return `<section class="section resource-hub"><div class="section-head"><p class="kicker">SEARCH INTENT ARCHITECTURE</p><h2>${esc(data.ui.related)}</h2><p>${esc(data.ui.sourceNote)}</p></div><div class="resource-grid">${guides}${tools}</div></section>`;
}
...
function pageHeader(locale, currentPath = "") {
  ...
  <header class="header inner-header">
    <a class="brand" href="/${locale}/"><span>×</span><b>DEAR PASSENGERS<small>CREW INTELLIGENCE</small></b></a>
    <button class="menu-button" aria-label="Menu">MENU</button>
    <nav><a href="/${locale}/release-date/">${esc(data.pages["release-date"].name)}</a><a href="/${locale}/demo/">${esc(data.pages.demo.name)}</a><a href="/${locale}/multiplayer/">${esc(data.pages.multiplayer.name)}</a><a href="/${locale}/system-requirements/">${esc(data.pages["system-requirements"].name)}</a></nav>
...
function pageFooter(locale) {
  const data = SEO[locale] || SEO.en;
  return `<footer><a class="brand" href="/${locale}/"><span>×</span><b>DEAR PASSENGERS<small>CREW INTELLIGENCE</small></b></a><p>${esc(copy[locale].footer)}</p><p>${esc(data.ui.updated)} · ${LAST_CHECKED}</p></footer>`;
}
...
function intentPage(locale, slug) {
  ...
  <link rel="canonical" href="${canonical}">${alternateLinks(slug)}<link rel="alternate" hreflang="x-default" href="${SITE}/en/${slug}/">
  ...
  <main class="inner-main">
    ...
    <section class="article-body">
      <article>
        ...
        <section><p class="kicker">04 · FAQ</p><h2>${esc(data.ui.faq)}</h2><div class="question-list">${faqs}</div></section>
      </article>
      <aside><div class="aside-card"><span>${esc(data.ui.current)}</span><strong>${esc(item.answer)}</strong></div><div class="aside-card sources-card"><span>${esc(data.ui.sources)}</span><a href="${STEAM}" target="_blank" rel="noopener">${esc(data.ui.official)} ↗</a><a href="https://www.youtube.com/watch?v=hEsuA_rqTxk" target="_blank" rel="noopener">${esc(data.ui.trailer)} ↗</a></div></aside>
    </section>
